...
// ---- Render functions (defined before DOMContentLoaded) ----
...
function renderChampionActions() {
  ...
  if (!championId) {
    ...
    btn.addEventListener('click', async ()=> { await set(ref(db, 'championId'), null); });
    area.appendChild(btn);
  }
}
...
function renderChampionBoard() {
  ...
  // champion board populated by DB listener below
}
...
function openChallengeModal(challengerId) {
  ...
  const winBtn = document.createElement('button'); winBtn.textContent = `${challenger.name} wins`; winBtn.style.background='#2b8a3e'; winBtn.style.color='#fff'; winBtn.addEventListener('click', async ()=>{ await submitChallenge(challengerId, desc.value||'', challengerId); close(); });
  const champBtn = document.createElement('button'); champBtn.textContent = `${players[championId].name} wins`; champBtn.style.background='#1f6feb'; champBtn.style.color='#fff'; champBtn.addEventListener('click', async ()=>{ await submitChallenge(challengerId, desc.value||'', championId); close(); });
  const cancel = document.createElement('button'); cancel.textContent='Cancel'; cancel.addEventListener('click', close);
  btnRow.append(winBtn, champBtn, cancel); panel.appendChild(btnRow); modal.appendChild(panel); document.body.appendChild(modal);
  function close(){ const m=$('challenge-modal'); if(m) m.remove(); }
}
...
async function addPlayer() {
  ...
  const name = input.value.trim(); if (!name) { alert('Enter a name'); return; }
...
document.addEventListener('click', (e)=>{}); // keep DOM events stable
...
onValue(ref(db, 'championBoard'), snap => {
  // optional: populate champion board
  renderChampionBoard();
});

onValue(ref(db, 'timer/endTimestamp'), snap => {
  timerEnd = snap.val() || null;
  isPendingState = !timerEnd;
  updateTimerDisplay();
});

// Connectivity check (optional)
(async function testConn(){
  try { const root = await get(ref(db, '/')); console.log('Initial DB root', root.val()); } catch (e) { console.error('Firebase connectivity test failed', e); }
})();
